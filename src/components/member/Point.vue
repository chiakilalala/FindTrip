<template>
  <div>
    <!-- 新增按鈕 -->
    <div class="mt-1 mb-3 flex justify-end" @click="dialogVisible = true">
      <el-button
        type="primary"
      ...
      <div class="lg:flex">
        ...
        <div class="min-w-full shadow-md rounded-lg block w-full overflow-x-auto">
          <table class="border-collapse table-auto w-full leading-normal text-left">
            ...
            <tbody>
              <tr class="whitespace-no-wrap" v-for="(item,index) in  points" :key="index">
                <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">{{item.CreateOn.slice(0,10)}}</td>
                <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">{{item.CreateOn.slice(11,16)}}</td>
                <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">{{item.Product}}點數</td>
              </tr>
    ...
    <el-dialog title="點數儲值" :visible.sync="dialogVisible" width="80%">
      <ul class="mt-2">
        <li class>
          <a href="#" class="block p-5 bg-gray-200 rounded shadow">
