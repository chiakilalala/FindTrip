<template>
  <div>
    <!-- 書籤 -->
    <div class="flex-1 text-gray-700 text-left mb-20 mt-10">
      <div class="min-w-full shadow-md rounded-lg block w-full overflow-x-auto">
        <table class="border-collapse table-auto w-full leading-normal text-left">
          <thead>
            <tr class="whitespace-no-wrap">
              <th
                class="px-5 py-3 border-b-2 border-gray-300 bg-gray-300 text-left text-base font-semibold text-gray-700 tracking-wider"
              >編號</th>
              <th
                class="px-5 py-3 border-b-2 border-gray-200 bg-gray-300 text-left text-base font-semibold text-gray-700 tracking-wider"
              >名字</th>

              <th
                class="px-5 py-3 border-b-2 border-gray-200 bg-gray-300 text-left text-base font-semibold text-gray-700 tracking-wider"
              >訊息內容</th>

              <th
                class="px-5 py-3 border-b-2 border-gray-200 bg-gray-300 text-left text-base font-semibold text-gray-700 tracking-wider"
              >時間</th>
            </tr>
          </thead>
          <tbody>
            <tr class="whitespace-no-wrap">
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">1</td>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">Adam</td>

              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">2020.2.13</td>

              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">2020.2.13</td>
            </tr>
            <tr>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">1</td>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">Adam</td>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">不拉多</td>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">2020.2.13</td>
            </tr>
            <tr>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">3</td>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">Chris</td>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">克布奇克多拉ｓ</td>
              <td class="px-5 py-3 border-b border-gray-200 bg-white text-base">2020.2.13</td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</template>

<script>
export default {};
</script>